...
create index if not exists idx_patch_list on patch (tenant_id, product_line_id, current_state, updated_at desc);
create index if not exists idx_patch_owner on patch (tenant_id, owner_pm_id, current_state);
...
create index if not exists idx_transition_patch_time on patch_transition_log (patch_id, created_at desc);
...
create index if not exists idx_operation_biz on patch_operation_log (tenant_id, biz_type, biz_id, created_at desc);
...
create index if not exists idx_kpi_rule_match on kpi_rule (tenant_id, stage, gate_type, enabled, priority);
...
create index if not exists idx_kpi_metric_patch on kpi_metric_value (tenant_id, patch_id, metric_key, collected_at desc);
...
create index if not exists idx_kpi_eval_patch on kpi_evaluation (tenant_id, patch_id, evaluated_at desc);
...
create index if not exists idx_kpi_detail_eval on kpi_evaluation_detail (evaluation_id, rule_id);
...
create index if not exists idx_qa_policy_match on qa_policy (tenant_id, stage, enabled, scope_type, scope_value);
...
create index if not exists idx_qa_pending on qa_task (tenant_id, assignee_id, status, stage);
create index if not exists idx_qa_patch on qa_task (patch_id, stage, qa_level);
...
create table if not exists test_task (
    ...
    case_prepare_rate double precision,
    env_ready boolean,
    case_execution_rate double precision,
    defect_density double precision,
...
create table if not exists review_session (
    ...
    quorum_required double precision,
    approve_rate_required double precision,
    status varchar(16) not null,
